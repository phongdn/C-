...
void FitnessAppWrapper::loadWeeklyExcercisePlan() // this will call the private loadWeeklyExcercisePlan()Plan() function
{
	...
	else
	{
		...
		this->loadWeeklyExcercisePlan(this->mFstrEP, this->mEPList);
		this->mFstrEP.close();
	}
}

void FitnessAppWrapper::displayWeeklyExcercisePlan() // this will call the private displayWeeklyExcercisePlan()Plan() function
{
	this->displayWeeklyExcercisePlan(this->mEPList);
}

void FitnessAppWrapper::storeWeeklyExcercisePlan() // this will call the private storeWeeklyExcercisePlan()Plan() function
{
	if (this->mFstrEP.is_open())
		this->mFstrEP.close();
	else
	{
		this->mFstrEP.open("exercisePlans.txt");
		this->storeWeeklyExcercisePlan(this->mFstrEP, this->mEPList);
		this->mFstrEP.close();
	}
}
...
//------------------------------Private Functions for DietPlan Operations ------------------------------------------
...
//------------------------------Private Functions for ExercisePlan Operations ------------------------------------------

void FitnessAppWrapper::loadDailyExcercisePlan(fstream &fileStream, ExcercisePlan &plan)
{
	fileStream >> plan;
}
void FitnessAppWrapper::loadWeeklyExcercisePlan(fstream &fileStream, ExcercisePlan weeklyPlan[])
{
	for (int day = 0; day < 7; ++day) 
	{
		this->loadDailyExcercisePlan(fileStream, weeklyPlan[day]); 
		char tempString[2];
		fileStream.getline(tempString, 2); 
	}
}
void FitnessAppWrapper::displayDailyExcercisePlan(const ExcercisePlan &plan)
{
	cout << plan << endl;
}
void FitnessAppWrapper::displayWeeklyExcercisePlan(const ExcercisePlan weeklyPlan[])
{
	for (int day = 0; day < 7; ++day) 
	{
		this->displayDailyExcercisePlan(weeklyPlan[day]); 
		cout << endl; 
	}
}
void FitnessAppWrapper::storeDailyExcercisePlan(fstream &fileStream, const ExcercisePlan &plan)
{
	fileStream << plan << endl;
}
void FitnessAppWrapper::storeWeeklyExcercisePlan(fstream &fileStream, const ExcercisePlan weeklyPlan[])
{
	for (int day = 0; day < 7; ++day) 
	{
		this->storeDailyExcercisePlan(fileStream, weeklyPlan[day]); 
		fileStream << endl; 
	}
}
